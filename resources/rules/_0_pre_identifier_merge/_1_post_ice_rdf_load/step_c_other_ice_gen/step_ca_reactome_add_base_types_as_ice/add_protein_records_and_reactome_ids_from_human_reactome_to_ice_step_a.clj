`{:description "This rule finds any protein record described in Reactome and traces out its link to a unification xref to extract the protein's Reactome ID.  Several Reactome protein records may correspond to the same UniProt ID, but the different records represent the protein in different modified forms or in different cellular compartments.",
 ...
 :reify ([?/reactome_ice] {:ns "http://ccp.ucdenver.edu/kabob/ice/"}
         [?/record_set {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 "Reactome v60 record set"), :prefix "R_"}]
 ...
 :head ((?/record_set rdf/type ccp/IAO_EXT_00000012)
        ...
        (?/reactome_ice rdfs/subClassOf ccp/IAO_EXT_0001643)
        (?/reactome_ice rdfs/subClassOf ccp/IAO_EXT_0001699)
        (?/reactome_ice rdfs/subClassOf ccp/IAO_EXT_0000088)
        (?/xref_id_field rdf/type ?/reactome_ice)
        
        ),
 :sparql-string "#add_protein_records_and_reactome_ids_from_human_reactome_to_ice_step_a.clj
...
SELECT ?prot ?xref ?react_db ?react_id ?clean_react_id ?reactome_ice
WHERE {
...
bind (uri (concat (str (\"http://ccp.ucdenver.edu/kabob/ice/REACTOME_\"), str (?clean_react_id))) AS ?reactome_ice) .
