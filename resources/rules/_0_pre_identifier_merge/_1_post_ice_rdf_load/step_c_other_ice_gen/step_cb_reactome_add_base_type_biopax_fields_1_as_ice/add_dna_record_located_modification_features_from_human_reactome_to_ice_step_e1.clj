{:description "This rule finds any previously reified DNA record from Reactome and traces out any bits of the DNA that have been cleaved (not its sequence modifications, which are described separately) when those are annoyingly described as modifications.",
 :name "add_dna_record_located_modification_features_from_human_reactome_to_ice_step_e1",
 ...
 :sparql-string "#add_dna_record_located_modification_features_from_human_reactome_to_ice_step_e1.clj
...
SELECT ?dna_record ?start ?end ?c ?frag_feature 
WHERE {
