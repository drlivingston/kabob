`{:description "This rule finds any right field described in Reactome; these describe inputs to biochemical reactions.",
 :name "add_reactome_rights_to_ice",
 :head ((?/reactome_thing_record obo/BFO_0000051 ?/right_thing_record)
        (?/right_thing_record rdf/type ccp/IAO_EXT_0001550) ;; right field
        ),
...
SELECT ?reactome_thing_record ?right_thing_record WHERE {
...
OPTIONAL {
 ?right_thing_record ccp:ekws_temp_biochemical_reaction_participant_relation ?left_thing_record .
...
filter (!bound (?left_thing_record)) .
}",
