`{:description "This rule finds any rna reference record described in Reactome with an existing miRBase ID.",
 :name "add_reactome_rna_references_with_existing_mirbase_ids_to_ice",
 :reify ([?/rna_ref_record {:ns "http://ccp.ucdenver.edu/kabob/ice/", :ln (:sha-1 "Reactome rna reference record" ?/rna_ref), :prefix "R_"}]
  ...
  :head ((?/rna_ref_record rdf/type ccp/IAO_EXT_0001561) ;; rna reference
         ...
         (?/xref_id_field rdf/type ?/mirbase_ice)
         (?/xref_id_field rdf/type ccp/IAO_EXT_0001594) ;; reactome mirbase id field
         (?/rna_ref ccp/ekws_temp_biopax_connector_relation ?/rna_ref_record)),
  :body "#add_reactome_rna_references_with_existing_mirbase_ids_to_ice.clj
...
SELECT ?rna_ref ?xref_record ?xref_id_field ?mirbase_ice WHERE {
...
?xref_id_field rdfs:label ?mirbase_id .
?xref_record obo:BFO_0000051 ?xref_db_field .
?xref_db_field rdf:type ccp:IAO_EXT_0001519 .
?xref_db_field rdfs:label \"miRBase\"@en .
bind (uri (concat (str (\"http://ccp.ucdenver.edu/kabob/ice/MIRBASE_\"), str (?mirbase_id))) as ?mirbase_ice) . 
