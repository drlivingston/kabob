...
;; -------------------------------------------
`{:name "step-hcc_missing-snrna-gen"
  ...
  select ?gene_bioentity ?rna_type ?has_gene_template ?rna_label {
          ...
          {
           select ?rna_type {
              kice:SO_0000274 obo:IAO_0000219 ?rna_type . # OBO:denotes
          ...
          ?gene_bioentity rdfs:subClassOf kice:temp_snrna_entity .
          optional {?gene_bioentity rdfs:label ?label}
  bind(coalesce(?label, \"Unnamed gene\") as ?gene_name)
  bind(concat(\"snRNA encoded by gene: \", str(?gene_name)) as ?rna_label)
