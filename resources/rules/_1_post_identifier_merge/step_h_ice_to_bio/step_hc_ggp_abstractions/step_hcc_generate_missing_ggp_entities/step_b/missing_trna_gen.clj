...
;; -------------------------------------------
`{:name "step-hcc_missing-trna-gen"
  :description "This rule asserts a tRNA entity for genes that are known to code for them."
  ...
  select ?gene_bioentity ?rna_type ?has_gene_template ?rna_label {
          ...
          {
           select ?rna_type {
              kice:SO_0000253 obo:IAO_0000219 ?rna_type . # OBO:denotes
              filter (?rna_type != obo:SO_0000253) # OBO:rRNA
              }
                             }
            {
           ...
           ?gene_bioentity rdfs:subClassOf kice:temp_trna_entity .
           optional {?gene_bioentity rdfs:label ?label}
  bind(coalesce(?label, \"Unnamed gene\") as ?gene_name)
  bind(concat(\"tRNA encoded by gene: \", str(?gene_name)) as ?rna_label)
