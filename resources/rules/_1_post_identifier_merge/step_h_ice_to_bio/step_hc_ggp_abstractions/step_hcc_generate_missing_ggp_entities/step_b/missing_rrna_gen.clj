...
;; -------------------------------------------
`{:name "step-hcc_missing-rrna-gen"
  :description "This rule asserts a rRNA entity for genes that are known to code for them."
  ...
  select ?gene_bioentity ?rna_type ?has_gene_template ?rna_label {
          ...
          {
           select ?rna_type {
              kice:SO_0000252 obo:IAO_0000219 ?rna_type . # OBO:denotes
              filter (?rna_type != obo:SO_0000252) # OBO:rRNA
              }
                             }
           {
          ...
          ?gene_bioentity rdfs:subClassOf kice:temp_rrna_entity .
  optional {?gene_bioentity rdfs:label ?label}
  bind(coalesce(?label, \"Unnamed gene\") as ?gene_name)
  bind(concat(\"rRNA encoded by gene: \", str(?gene_name)) as ?rna_label)
