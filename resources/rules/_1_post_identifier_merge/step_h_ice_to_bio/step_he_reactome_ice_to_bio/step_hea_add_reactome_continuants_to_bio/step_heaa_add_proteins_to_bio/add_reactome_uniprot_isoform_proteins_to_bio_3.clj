...
The rule generates the Reactome protein's ICE identifier, and places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the UniProt ICE id.",
 :name "add_reactome_uniprot_isoform_proteins_to_bio_3",
 ...
 :body "#add_reactome_uniprot_isoform_proteins_to_bio_3
...
WHERE {
?uniprot_field rdf:type ccp:IAO_EXT_0001599 .  # Reactome uniprot isoform accession field
?uniprot_field rdfs:label ?isoform_accession_label .
...
bind (uri (concat (str (\"http://purl.obolibrary.org/obo/\"), str (?isoform_accession_label))) AS ?obo_uniprot) .
filter (?obo_uniprot != ?uniprot_bio) .
bind (strbefore (str (?isoform_accession_label), \"-\") as ?accession_label) .
bind (strlang (?accession_label, \"en\") as ?main_isoform_accession_label) .

# match the reactome to the uniprot via the accession label
?uniprot_accession_field rdfs:label ?main_isoform_accession_label .
...
filter (contains (str (?gocc_ice), \"http://ccp.ucdenver.edu/kabob/ice/GO_\")) .# either the protein has a sequence modification 
OPTIONAL {
...
filter (bound (?modification_feature) || (bound (?fragment_feature) && (?start_coord > ?one || ?end_coord < ?seq_length))) .
