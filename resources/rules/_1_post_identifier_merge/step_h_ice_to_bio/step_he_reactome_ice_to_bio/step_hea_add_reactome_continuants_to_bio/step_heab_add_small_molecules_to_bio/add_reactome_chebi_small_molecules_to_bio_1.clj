`{:description "This rule finds any ChEBI small molecule record described in Reactome.  The rule generates the Reactome small molecule's ICE identifier, and places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the ChEBI ICE id.",
 :name "add_reactome_chebi_small_molecules_to_bio_1",
 ...
 :head ((?/reactome_ice obo/IAO_0000219 ?/original_foo)
        (?/original_foo rdfs/subClassOf ?/chebi_bio)
...
SELECT DISTINCT ?reactome_ice ?chebi_bio ?transports_bio ?targets_bio ?localization_bio ?gocc_bio ?small_molecule_record 
WHERE {
...
?xref_record obo:BFO_0000051 ?chebi_field .
?chebi_field rdf:type ccp:IAO_EXT_0001940 . # Reactome ChEBI field
?chebi_field rdfs:label ?accession_label .
bind (strafter (str (?accession_label), \":\") as ?chebi_accession) .
?chebi_field rdf:type ?chebi_ice .
filter (contains (str (?chebi_ice), \"http://ccp.ucdenver.edu/kabob/ice/CHEBI_\")) .
?chebi_ice obo:IAO_0000219 ?chebi_bio .
bind (uri (concat (str (\"http://purl.obolibrary.org/obo/CHEBI_\"), str (?chebi_accession))) AS ?obo_chebi) .
filter (?obo_chebi != ?chebi_bio) .
