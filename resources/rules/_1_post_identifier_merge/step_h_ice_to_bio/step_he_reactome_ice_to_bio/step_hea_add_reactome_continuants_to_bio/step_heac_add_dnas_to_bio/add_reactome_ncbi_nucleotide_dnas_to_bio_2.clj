`{:description "This rule finds any modified NCBI Nucleotide dna record described in Reactome.  The rule generates the Reactome dna's ICE identifier, and places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the NCBI Nucleotide ICE id.  It also makes the denotational link to a new NCBI Nucleotide BIO entity if one doesn't exist already.",
 ...
 :reify ([?/modified_foo {:ln (:sha-1 "bio-side modified reactome dna" ?/dna_record), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
         ...
         [?/ncbi_nucleotide_bio {:ln (:sha-1 "NCBI Nucleotide DNA" ?/accession_label), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
 ...
 :body "#add_reactome_ncbi_nucleotide_dnas_to_bio_1
...
SELECT DISTINCT ?reactome_ice ?ncbi_nucleotide_ice ?accession_label ?transports_bio ?targets_bio ?localization_bio ?gocc_bio ?dna_record ?variant_of_bio
WHERE {
...
# one of these marks the DNA as a variant
