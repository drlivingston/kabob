`{:description "This rule finds any modified ENSEMBL rna record described in Reactome.  The rule generates the Reactome rna's ICE identifier, and places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the ENSEMBL ICE id.  It also makes the denotational link to a new ENSEMBL BIO entity if one doesn't exist already.",
 ...
 :reify ([?/modified_foo {:ln (:sha-1 "bio-side modified reactome rna" ?/rna_record), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
         ...
         [?/ensembl_bio {:ln (:sha-1 "ENSEMBL RNA" ?/accession_label), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
 ...
 :body "#add_reactome_ensembl_rnas_to_bio_2
...
SELECT DISTINCT ?reactome_ice ?ensembl_ice ?accession_label ?transports_bio ?targets_bio ?localization_bio ?gocc_bio ?rna_record ?variant_of_bio
WHERE {
