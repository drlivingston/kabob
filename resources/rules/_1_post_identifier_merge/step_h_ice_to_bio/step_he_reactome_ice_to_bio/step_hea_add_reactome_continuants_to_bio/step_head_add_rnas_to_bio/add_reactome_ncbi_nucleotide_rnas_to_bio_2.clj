`{:description "This rule finds any modified NCBI Nucleotide rna record described in Reactome.  The rule generates the Reactome rna's ICE identifier, and places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the NCBI Nucleotide ICE id.  It also makes the denotational link to a new NCBI Nucleotide BIO entity if one doesn't exist already.",
 :name "add_reactome_ncbi_nucleotide_rnas_to_bio_1",
 :reify ([?/modified_foo {:ln (:sha-1 "bio-side modified reactome rna" ?/rna_record), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
         ...
         [?/ncbi_nucleotide_bio {:ln (:sha-1 "NCBI Nucleotide RNA" ?/accession_label), :ns "http://ccp.ucdenver.edu/kabob/bio/" :prefix "B_"}]
 ...
 :head ((?/reactome_ice obo/IAO_0000219 ?/modified_foo)
        (?/ncbi_nucleotide_ice obo/IAO_0000219 ?/ncbi_nucleotide_bio)
        (?/original_foo rdfs/subClassOf ?/ncbi_nucleotide_bio)
 ...
 :body "#add_reactome_ncbi_nucleotide_rnas_to_bio_1
...
SELECT DISTINCT ?reactome_ice ?ncbi_nucleotide_ice ?accession_label ?transports_bio ?targets_bio ?localization_bio ?gocc_bio ?rna_record ?variant_of_bio
WHERE {
...
?xref_record obo:BFO_0000051 ?ncbi_nucleotide_field .
?ncbi_nucleotide_field rdf:type ccp:IAO_EXT_0001597 . # Reactome NCBI Nucleotide field
?ncbi_nucleotide_field rdfs:label ?accession_label .
?ncbi_nucleotide_field rdf:type ?ncbi_nucleotide_ice .
filter (contains (str (?ncbi_nucleotide_ice), \"http://ccp.ucdenver.edu/kabob/ice/NCBI_\")) .
bind (uri (concat (str (\"http://purl.obolibrary.org/obo/GENBANK_\"), str (?accession_label))) AS ?obo_ncbi_nucleotide) .
OPTIONAL {
?ncbi_nucleotide_ice obo:IAO_0000219 ?ncbi_nucleotide_bio .
?ncbi_nucleotide_ice obo:IAO_0000219 ?obo_ncbi_nucleotide .
filter (?obo_ncbi_nucleotide != ?ncbi_nucleotide_bio) .
}
filter (!bound (?ncbi_nucleotide_bio)) .
# make sure there this is a RNA variant
OPTIONAL {
...
?fragment_feature_record rdf:type ccp:IAO_EXT_0001587 . # fragment features allowed
}
# one of these marks the RNA as a variant
