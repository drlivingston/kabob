`{:description "This rule finds any modified miRBase rna record described in Reactome.  The rule generates the Reactome rna's ICE identifier, and places the top-level entity on the BIO side as a localized subclass of the BIO entity denoted by the miRBase ICE id.  It also makes the denotational link to a new miRBase BIO entity if one doesn't exist already.",
 ...
 :body "#add_reactome_mirbase_rnas_to_bio_2
...
SELECT DISTINCT ?reactome_ice ?mirbase_ice ?accession_label ?transports_bio ?targets_bio ?localization_bio ?gocc_bio ?rna_record ?variant_of_bio
WHERE {
