...
;; ------------------------------------------------------------------------
`{:name          "step-ga_copy-anonymous-nodes-to-bio"
  ...
  :head          ((?/anon_record rdf/type ccp/IAO_EXT_0001707) ; ccp:anonymous_node_record
                   ...
                   (?/bio_blank_node rdf/type ?/type)) ; IAO:denotes
  ...
  PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                  ...
                  select distinct ?blank_node ?type {
                      graph ?g {
                                ?blank_node rdf:type ?type .
                                filter      (contains (str (?blank_node), '/bnode/'))
                                filter (?type != owl:Axiom)
                                minus {?blank_node ?p ?c .
