...
echo Generating new KB
echo Loading list file: $LIST_FILE
cat $LIST_FILE


$AG_BIN/agload -v --bulk --supersede --graph :source --with-indices "$DEFAULT_KB_INDICES" -p $PORT --input $FORMAT $KB @$LIST_FILE

#--with-indices "ospgi posgi spogi"  --encoding utf8

$SCRIPT_DIR/remove-duplicates.sh $AG_BIN $PORT $KB $KABOB_DATA_ROOT
