...
export LOAD_REQUEST_DIRECTORY=/stardog-load-requests

# retrieve the AllegroGraph port being used from $LOAD_REQUEST_DIRECTORY/agraph.port
export PLATFORM_KB_PORT=$(cat "${LOAD_REQUEST_DIRECTORY}/stardog.port")
export CONTAINER_NAME=$(cat "${LOAD_REQUEST_DIRECTORY}/stardog.container.name")
...
export KB_INSTANCE_DATA_DIR=/kabob_data/stardog-build
